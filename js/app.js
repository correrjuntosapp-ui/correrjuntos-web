  ...
  if(!window.__CJ_SUPABASE_READY__){
    window.__CJ_SUPABASE_READY__ = (async () => {
      ...
      try{
        ...
        console.log('🔑 getSession result:', { hasSession: !!(data && data.session), isRecovery: __cjIsRecovery });
        ...
        if (!data?.session && !__cjIsRecovery) {
          ...
          if (hashStr.includes('access_token=')) {
            console.log('🔑 Implicit flow: hash contiene access_token, intentando setSession manual...');
            try {
              ...
              if (at && rt) {
                ...
                console.log('🔑 setSession manual result:', { hasSession: !!(sessData && sessData.session), error: sessErr?.message });
                if (!sessErr && sessData?.session) {
                  // Limpiar hash de la URL
                  try { window.history.replaceState({}, document.title, window.location.pathname + window.location.search); } catch(_) {}
                  // Hidratar y mostrar app
                  await hydrateUserFromSession();
                  console.log('🔑 After manual OAuth hydrate, currentUser:', !!window.currentUser);
